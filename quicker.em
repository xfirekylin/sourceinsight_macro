...
macro InsertFileHeaderCN(hbuf, ln,szName,szContent)
{
    ...
    InsBufLine(hbuf, ln + 0,  "/******************************************************************************")
    InsBufLine(hbuf, ln + 1,  "")
    InsBufLine(hbuf, ln + 2,  "                  版权所有 (C), 2013, 深圳市晨创通电子科技有限公司")
    InsBufLine(hbuf, ln + 3,  "")
    InsBufLine(hbuf, ln + 4,  " ******************************************************************************")
    sz = GetFileName(GetBufName (hbuf))
...
macro InsertStringByPos(sz, str, pos)
{
	len = strlen(sz);
...
macro IfdefStrExt(sz, new_s)
{
	...
	if (!sel.fExtended)
	{
		if (strlen(szLine) > nLeft+4)
		{
			headstr = strmid(szLine,nLeft, nLeft+4);

			if (headstr == "#if ")
			{
				szLine = InsertStringByPos(szLine, " @new_s@defined @sz@", sel.ichFirst)
				//szLine = cat(szLine, "|| defined @sz@")

				PutBufLine(hbuf,lnLast,szLine)
			    SetBufIns(hbuf,lnFirst,strlen(szLine))
				return
			}
		}

		if (strlen(szLine) > nLeft+7)
		{
			headstr = strmid(szLine,nLeft, nLeft+7);

			if (headstr == "#ifdef ")
			{
				headstr = strmid(szLine,nLeft+7, strlen(szLine));

				szLine = strmid(szLine,0,nLeft);
				szLine = cat(szLine, "#if defined ")
				szLine = cat(szLine, headstr)
				
				szLine = InsertStringByPos(szLine, " @new_s@defined @sz@", sel.ichFirst+5)
				//szLine = cat(szLine, " || defined @sz@")
		...
		if (strlen(szLine) > nLeft+8)
		{
			headstr = strmid(szLine,nLeft, nLeft+8);

			if (headstr == "#ifndef ")
			{
				headstr = strmid(szLine,nLeft+8, strlen(szLine));
		...
		if (strlen(szLine) > nLeft+6)
		{
			headstr = strmid(szLine,nLeft, nLeft+6);
    ...
    if(lnLast + 1 < lnMax)
    {
        InsBufLine(hbuf, lnLast+1, "@szLeft@#endif")
    }
...
macro IfdefStrOrN(sz)
{
	IfdefStrExt(sz, "|| !")
}


macro IfdefStrAnd(sz)
{
	IfdefStrExt(sz, "&& ")
}

macro IfdefStrAndN(sz)
{
	IfdefStrExt(sz, "&& !")
}

macro IfdefelseStr(sz)
{
    hwnd = GetCurrentWnd()
    lnFirst = GetWndSelLnFirst(hwnd)
    lnLast = GetWndSelLnLast(hwnd)
    ...
    InsBufLine(hbuf, lnFirst, "@szLeft@#ifdef @sz@")
	InsBufLine(hbuf, lnFirst+1, "@szLeft@")
	InsBufLine(hbuf, lnFirst+2, "@szLeft@#else")

}

macro ElseIfdefStr(sz)
{
    hwnd = GetCurrentWnd()
    lnFirst = GetWndSelLnFirst(hwnd)
    lnLast = GetWndSelLnLast(hwnd)
    ...
    InsBufLine(hbuf, lnFirst, "@szLeft@#elif defined @sz@")
    InsBufLine(hbuf, lnFirst+1, "@szLeft@    ")
    SetBufIns(hbuf,lnFirst + 3,strlen(szLeft))
}
...
macro PredefIfeStr()
{
    ...
    InsBufLine(hbuf, lnFirst, "@szLeft@#if  @sz@")
    InsBufLine(hbuf, lnFirst+1, "@szLeft@#else  @sz@")
...
//    if(strlen( szMyName ) == 0)
    {
        szMyName = "xiaolh"//Ask("Enter your name:")
        msg(szMyName)
        setreg(MYNAME, szMyName)
    }
       SetBufIns (hbuf, 0, 0)
...
macro InsertTopBottomStr(top,bottom)
{
    hwnd = GetCurrentWnd()
    lnFirst = GetWndSelLnFirst(hwnd)
    lnLast = GetWndSelLnLast(hwnd)
    ...
    if(lnLast + 1 < lnMax)
    {
        InsBufLine(hbuf, lnLast+1, "@szLeft@@bottom@")
    }
    else if(lnLast + 1 == lnMax)
    {
        AppendBufLine(hbuf, "@szLeft@@bottom@")
    }
    else 
    {
        AppendBufLine(hbuf, "")
